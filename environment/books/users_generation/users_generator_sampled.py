...
class UserGeneratorHardLLMFeatures:
    def __init__(self, llm):
        ...
        self.dist = pd.read_csv(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)),
                "../datasets/books_amazon/categories.csv",
            ),
            index_col=0,
        )
        self.dist["Category"] = self.dist.index
    ...
    def generate_liked_categories(self):
        return self.rng.choice(
            self.dist["Category"].values,
            size=4,
            p=self.dist["count"].values / self.dist["count"].values.sum(),
            replace=False,
        )

    def generate_not_like_categories(self, liked):
        remain = self.dist[self.dist["Category"].apply(lambda x: not x in liked)]
        return self.rng.choice(
            remain["Category"].values,
            size=4,
            p=remain["count"].values / remain["count"].values.sum(),
            replace=False,
        )
    ...
    def generate_normal(self, seed, num):
        ...
        for i in range(len(ages)):
            ...
            a = self.generate_liked_categories()
            b = self.generate_not_like_categories(a)
